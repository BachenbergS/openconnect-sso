...
def parse_auth_complete_response(xml):
    # Sicherstellen, dass die Authentifizierungs-ID "success" ist
    assert xml.auth.get("id") == "success", "Authentication failed: ID is not 'success'."
    
    # Versuche, das 'banner'-Attribut zu lesen
    try:
        if hasattr(xml.auth, "banner") and xml.auth.banner is not None:
            auth_message = xml.auth.banner.text
        else:
            # Fallback: Verwende das Attribut 'message', falls 'banner' fehlt
            auth_message = getattr(xml.auth, "message", "")
    except AttributeError as e:
        logger.warning(f"Optional attribute missing in response: {e}")
        auth_message = "No message available."
    
    # Extrahiere andere Attribute aus dem XML
    try:
        ...
        logger.error(f"Missing expected key in XML response: {e}")
        raise ValueError(f"Invalid XML response: missing key {e}")
